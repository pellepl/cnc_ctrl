...
import comm.pelleplutt.util.Essential;
import comm.pelleplutt.util.Log;

/**
 * Manages the opening and closing of a serial port, returning inputstream and
 * outputstream to the resource. Also takes care of watchdog feature would there
 * be an I/O operation that times out. Designed for single thread access to the
 * port, i.e. having multiple threads calling read or write simultaneously will
 * cause weird effects. Different threads in different points in time is ok.
 * 
 * @author petera
 */
public abstract class PortConnector {
	...
	public static PortConnector getPortConnector() {
		...
		if (os.contains("Windows")) {
			if (System.getProperty(WinSerialPortUARTSocket.PROP_PATH_BIN) == null) {
				System.setProperty(WinSerialPortUARTSocket.PROP_PATH_BIN,
						System.getProperty("user.home") + File.separatorChar
								+ Essential.userSettingPath + File.separatorChar
								+ "bin");
			}
			...
			if (System.getProperty(LinuxSerialPortUARTSocket.PROP_PATH_BIN) == null) {
				System.setProperty(LinuxSerialPortUARTSocket.PROP_PATH_BIN,
						System.getProperty("user.home") + File.separatorChar
								+ Essential.userSettingPath + File.separatorChar
								+ "bin");
			}
			if (System.getProperty(LinuxSerialPortUARTSocket.PROP_PATH_SRC) == null) {
				System.setProperty(LinuxSerialPortUARTSocket.PROP_PATH_SRC,
						System.getProperty("user.home") + File.separatorChar
								+ Essential.userSettingPath + File.separatorChar
								+ "src");
			}
	 ...
	 * @throws IOException
	 */
	public abstract void setRTSDTR(boolean rtshigh, boolean dtrhigh) throws IOException;
	...
	protected void setInputStream(InputStream inputStream) {
		this.inputStream = new PushbackInputStream(/*new WatchedInputStream*/(inputStream), 1);
	}
			...
			synchronized (monitor) {
				while (monitor.running) {
					try {
						...
						} else if (active && timeoutSetting > 0 && now - then >= timeoutSetting-10) {
							// timed out!
